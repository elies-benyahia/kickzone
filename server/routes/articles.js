// routes/articles.js — URL des articles -> contrôleur. Écriture réservée aux admins.
...
router.get('/', list);                          // liste paginée (?category, ?page, ?limit)

// Recherche plein texte dans titre / résumé (5 résultats max).
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT id, slug, title, summary, image_url AS imageUrl, category, published_at AS publishedAt
       FROM articles WHERE title LIKE ? OR summary LIKE ? ORDER BY published_at DESC LIMIT 5`,
      [`%${q}%`, `%${q}%`]
    );
    res.json(rows);
  } catch (e) { next(e); }
...
router.get('/:slug', get);                                              // un article (+ 1 vue)
