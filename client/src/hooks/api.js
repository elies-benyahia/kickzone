// hooks/api.js — tous les échanges avec l'API. 1 hook = 1 endpoint.
// React Query gère le cache, le rechargement et les états (isLoading, data, error).

import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import axios from 'axios';

// Client HTTP unique. En prod VITE_API_URL = "/api" (même serveur).
const api = axios.create({ baseURL: import.meta.env.VITE_API_URL || 'http://localhost:3001/api' });

// Avant chaque requête : ajoute le jeton JWT s'il existe.
...
// GET qui renvoie directement les données.
const get = (url, params) => api.get(url, { params }).then((r) => r.data);

export { api }; // utilisé par AuthContext

const MIN = 60 * 1000;

// Rafraîchit un match toutes les minutes s'il est en direct, jamais s'il est fini.
const LIVE = ['1H', '2H', 'HT', 'ET', 'P', 'LIVE', 'INT'];
const refetchLive = (data) => {
  const f = Array.isArray(data) ? data[0] : data;
  return LIVE.includes(f?.fixture?.status?.short) ? MIN : false;
};

// Rappel : queryKey = clé de cache, queryFn = fonction qui charge, enabled = condition.
...
// Recherche globale : joueurs + équipes + articles (à partir de 2 lettres).
...
// useMutation = envoyer des données (POST). Après succès, on recharge la liste.
