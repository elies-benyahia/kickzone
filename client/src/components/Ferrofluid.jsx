...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (uMouseEnabled > 0.5) {
    ...
    float md = length(p - mp)/ref; float rr = max(uMouseRadius, 0.02);
...
const Ferrofluid = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const call = (obj, key) => { const fn = obj?.[key]; if (typeof fn === 'function') fn.call(obj); };
      call(programRef.current, 'remove');
      call(geometryRef.current, 'remove');
      call(meshRef.current, 'remove');
      call(rendererRef.current, 'destroy');
  ...
  return (
    <div ref={containerRef} className={`ferrofluid-container ${className ?? ''}`}
