// MatchCard — une ligne de match (domicile / score ou heure / extérieur), cliquable vers /match/:id.
...
export default function MatchCard({ fixture }) {
  const { fixture: f, teams, goals } = fixture; // format de l'API-Football
  ...
  return (
    <Link to={`/match/${f.id}`} className={styles.card}>
      ...
      {/* minute si live, score si fini, heure sinon */}
      <div className={styles.center}>
        {isLive ? <LiveBadge minute={f.status.elapsed} />
          : isFinished ? <span className={styles.score}>{goals.home ?? 0} – {goals.away ?? 0}</span>
